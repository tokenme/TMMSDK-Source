...
#ifndef TMMWeakTimer_h
#define TMMWeakTimer_h

FOUNDATION_EXPORT double TMMWeakTimerVersionNumber;
FOUNDATION_EXPORT const unsigned char TMMWeakTimerVersionString[];

/**
 `TMMWeakTimer` behaves similar to an `NSTimer` but doesn't retain the target.
 This timer is implemented using GCD, so you can schedule and unschedule it on arbitrary queues (unlike regular NSTimers!)
 */
@interface TMMWeakTimer : NSObject
     ...
     * @param repeats if `YES`, `selector` will be invoked on `target` until the `TMMWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
- (id)initWithTimeInterval:(NSTimeInterval)timeInterval
    ...
    /**
     * Creates an `TMMWeakTimer` object and schedules it to start ticking inmediately.
     */
...
- (void)fire;
     ...
     * @note `TMMWeakTimer` won't invoke the `selector` on `target` again after calling this method.
