...
NSString * const TMMKeyChainStoreErrorDomain = @"io.tokenmama.tmmkeychainstore";
static NSString *_defaultService;

@interface TMMKeyChainStore ()

@end

@implementation TMMKeyChainStore
...
+ (TMMKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (TMMKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:TMMKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = TMMKeyChainStoreItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:TMMKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(TMMKeyChainStoreProtocolType)protocolType authenticationType:(TMMKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = TMMKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = TMMKeyChainStoreAccessibilityAfterFirstUnlock;
}
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    TMMKeyChainStore *keychain = [TMMKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray TMM_KEY_TYPE *)allKeysWithItemClass:(TMMKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == TMMKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == TMMKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(TMMKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == TMMKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray TMM_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
    [self requestSharedWebCredentialForDomain:nil account:nil completion:completion];
}

+ (void)requestSharedWebCredentialForDomain:(NSString *)domain account:(NSString *)account completion:(void (^)(NSArray TMM_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == TMMKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'TMMKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'TMMKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case TMMKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case TMMKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case TMMKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case TMMKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case TMMKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case TMMKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case TMMKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case TMMKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case TMMKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case TMMKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case TMMKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case TMMKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case TMMKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case TMMKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case TMMKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case TMMKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case TMMKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case TMMKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case TMMKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case TMMKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case TMMKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case TMMKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case TMMKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case TMMKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case TMMKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case TMMKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case TMMKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case TMMKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case TMMKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case TMMKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case TMMKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case TMMKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case TMMKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case TMMKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case TMMKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case TMMKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case TMMKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case TMMKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case TMMKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case TMMKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case TMMKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case TMMKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case TMMKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case TMMKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case TMMKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case TMMKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:TMMKeyChainStoreErrorDomain code:TMMKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:TMMKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:TMMKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:TMMKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation TMMKeyChainStore (Deprecation)
