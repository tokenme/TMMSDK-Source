...
#error The TMMJSONMOdel framework is ARC only, you can enable ARC on per file basis.
#endif
...
#import "TMMJSONModelClassProperty.h"
...
static TMMJSONValueTransformer* valueTransformer = nil;
static Class TMMJSONModelClass = NULL;

#pragma mark - model cache
static TMMJSONKeyMapper* globalKeyMapper = nil;

#pragma mark - JSONModel implementation
@implementation TMMJSONModel
{
...
+(void)load
{
    ...
    dispatch_once(&once, ^{
        ...
        // which are common for ALL TMMJSONModel subclasses
        ...
        @autoreleasepool {
            ...
            valueTransformer = [[TMMJSONValueTransformer alloc] init];

            // This is quite strange, but I found the test isSubclassOfClass: (line ~291) to fail if using [TMMJSONModel class].
            ...
            // Using NSClassFromString instead of [TMMJSONModel class], as this was breaking unit tests, see below
            //http://stackoverflow.com/questions/21394919/xcode-5-unit-test-seeing-wrong-class
            TMMJSONModelClass = NSClassFromString(NSStringFromClass(self));
        }
    });
}
...
-(instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err
{
    //check for nil input
    if (!data) {
        if (err) *err = [TMMJSONModelError errorInputIsNil];
        return nil;
    }
    //read the json
    ...
    if (initError) {
        if (err) *err = [TMMJSONModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithString:(NSString*)string error:(TMMJSONModelError**)err
{
    TMMJSONModelError* initError = nil;
...
-(id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(TMMJSONModelError**)err
{
    //check for nil input
    if (!string) {
        if (err) *err = [TMMJSONModelError errorInputIsNil];
...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [TMMJSONModelError errorInvalidDataWithMessage:@"Attempt to initialize TMMJSONModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [TMMJSONModelError errorModelIsInvalid];
        return nil;
    }
...
-(TMMJSONKeyMapper*)__keyMapper
{
    //get the model key mapper
    return objc_getAssociatedObject(self.class, &kMapperObjectKey);
}

-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(TMMJSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (TMMJSONModelClassProperty* property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        if (err) *err = [TMMJSONModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(TMMJSONKeyMapper*)keyMapper
{
    if (keyMapper) {
        //custom mapper
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(TMMJSONKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (TMMJSONModelClassProperty* property in [self __properties__]) {
        ...
        if (isNull(jsonValue)) {
            ...
            if (err) {
                ...
                TMMJSONModelError* dataErr = [TMMJSONModelError errorInvalidDataWithMessage:msg];
        ...
        if (property) {
            ...
            // 1) check if property is itself a TMMJSONModel
            if ([self __isTMMJSONModelSubClass:property.type]) {

                //initialize the property's model, store it
                TMMJSONModelError* initErr = nil;
                ...
                if (
                    ...
                    Class sourceClass = [TMMJSONValueTransformer classByResolvingClusterClasses:[jsonValue class]];
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            TMMJSONModelError* dataErr = [TMMJSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
-(BOOL)__isTMMJSONModelSubClass:(Class)class
{
// http://stackoverflow.com/questions/19883472/objc-nsobject-issubclassofclass-gives-incorrect-failure
#ifdef UNIT_TESTING
    return [@"TMMJSONModel" isEqualToString: NSStringFromClass([class superclass])];
#else
    return [class isSubclassOfClass:TMMJSONModelClass];
#endif
}
...
-(NSMutableSet*)__requiredPropertyNames
{
    ...
    if (!classRequiredPropertyNames) {
        ...
        [[self __properties__] enumerateObjectsUsingBlock:^(TMMJSONModelClassProperty* p, NSUInteger idx, BOOL *stop) {
...
-(void)__inspectProperties
{
    ...
    // inspect inherited properties up to the TMMJSONModel class
    while (class != [TMMJSONModel class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            TMMJSONModelClassProperty* p = [[TMMJSONModelClassProperty alloc] init];
...
#pragma GCC diagnostic pop
            ...
            //the property must be a primitive
                ...
                if (![allowedPrimitiveTypes containsObject:propertyType]) {
                    ...
                    @throw [NSException exceptionWithName:@"TMMJSONModelProperty type not allowed"
                                                   reason:[NSString stringWithFormat:@"Property type of %@.%@ is not supported by TMMJSONModel.", self.class, p.name]
                                                 userInfo:nil];
                }
            ...
            //few cases where TMMJSONModel will ignore properties automatically
            ...
            if (p)
            {
                ...
                {
                    NSString *class = NSStringFromClass([TMMJSONValueTransformer classByResolvingClusterClasses:type]);
        ...
        //(will do that until it reaches the root class - TMMJSONModel)
...
-(id)__transform:(id)value forProperty:(TMMJSONModelClassProperty*)property error:(NSError**)err
{
    Class protocolClass = NSClassFromString(property.protocol);
    if (!protocolClass) {

        //no other protocols on arrays and dictionaries
        //except TMMJSONModel classes
        if ([value isKindOfClass:[NSArray class]]) {
            @throw [NSException exceptionWithName:@"Bad property protocol declaration"
                                           reason:[NSString stringWithFormat:@"<%@> is not allowed TMMJSONModel property protocol, and not a TMMJSONModel class.", property.protocol]
                                         userInfo:nil];
        }
        return value;
    }

    //if the protocol is actually a TMMJSONModel class
    if ([self __isTMMJSONModelSubClass:protocolClass]) {
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
            ...
            if(![[value class] isSubclassOfClass:[NSArray class]])
            {
                if(err != nil)
                {
                    ...
                    TMMJSONModelError* typeErr = [TMMJSONModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            //one shot conversion
            TMMJSONModelError* arrayErr = nil;
...
-(id)__reverseTransform:(id)value forProperty:(TMMJSONModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
    ...
    if ([self __isTMMJSONModelSubClass:protocolClass]) {
        ...
        if (property.type == [NSArray class] || property.type == [NSMutableArray class]) {
            ...
            for (NSObject<AbstractTMMJSONModelProtocol>* model in (NSArray*)value) {
        ...
        if (property.type == [NSDictionary class] || property.type == [NSMutableDictionary class]) {
            ...
            for (NSString* key in [(NSDictionary*)value allKeys]) {
                id<AbstractTMMJSONModelProtocol> model = value[key];
...
- (BOOL)__customSetValue:(id <NSObject>)value forProperty:(TMMJSONModelClassProperty *)property
{
    NSString *class = NSStringFromClass([TMMJSONValueTransformer classByResolvingClusterClasses:[value class]]);
...
- (BOOL)__customGetValue:(id *)value forProperty:(TMMJSONModelClassProperty *)property
{
...
- (NSDictionary *)toDictionaryWithKeys:(NSArray <NSString *> *)propertyNames
{
    ...
    for (TMMJSONModelClassProperty* p in properties) {
        ...
        if ([value isKindOfClass:TMMJSONModelClass]) {

            //recurse models
            value = [(TMMJSONModel*)value toDictionary];
            [tempDictionary setValue:value forKeyPath: keyPath];
            ...
            if (YES) {
                ...
                if (foundCustomTransformer) {
                    ...
                    @throw [NSException exceptionWithName:@"Value transformer not found"
                                                   reason:[NSString stringWithFormat:@"[TMMJSONValueTransformer %@] not found", selectorName]
                                                 userInfo:nil];
                    return nil;
                }
            }
...
- (NSData *)toJSONDataWithKeys:(NSArray <NSString *> *)propertyNames
{
    ...
    @catch (NSException *exception) {
        //this should not happen in properly design TMMJSONModel
...
+ (NSMutableDictionary *)dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError **)err
{
    ...
    for (NSString *key in dictionary.allKeys)
    {
        ...
        {
            if (err) {
                *err = [TMMJSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
...
+(NSMutableArray*)arrayOfDictionariesFromModels:(NSArray*)array
{
    ...
    for (id<AbstractTMMJSONModelProtocol> object in array) {
...
+(NSMutableDictionary *)dictionaryOfDictionariesFromModels:(NSDictionary *)dictionary
{
    ...
    for (NSString *key in dictionary.allKeys) {
        id <AbstractTMMJSONModelProtocol> object = dictionary[key];
        id obj = [object toDictionary];
        if (!obj) return nil;
        output[key] = obj;
    }
...
-(NSString*)description
{
    ...
    for (TMMJSONModelClassProperty *p in [self __properties__]) {
...
+(TMMJSONKeyMapper*)keyMapper
{
    return nil;
}

+(void)setGlobalKeyMapper:(TMMJSONKeyMapper*)globalKeyMapperParam
{
