...
typedef NSString *(^TMMJSONModelKeyMapBlock)(NSString *keyName);
 ...
 *
 * In your model overwrite the + (TMMJSONKeyMapper *)keyMapper method and provide a TMMJSONKeyMapper
 * instance to convert the key names for your model.
 *
 * If you need custom mapping it's as easy as:
 * <pre>
 * + (TMMJSONKeyMapper *)keyMapper {
 * &nbsp; return [[TMMJSONKeyMapper&nbsp;alloc]&nbsp;initWithDictionary:@{@"crazy_JSON_name":@"myCamelCaseName"}];
 * }
 * </pre>
 * In case you want to handle underscore_case, **use the predefined key mapper**, like so:
 * <pre>
 * + (TMMJSONKeyMapper *)keyMapper {
 * &nbsp; return [TMMJSONKeyMapper&nbsp;mapperFromUnderscoreCaseToCamelCase];
 * }
 * </pre>
 */
@interface TMMJSONKeyMapper : NSObject

// deprecated
@property (readonly, nonatomic) TMMJSONModelKeyMapBlock TMMJSONToModelKeyBlock DEPRECATED_ATTRIBUTE;
...
- (instancetype)initWithJSONToModelBlock:(TMMJSONModelKeyMapBlock)toModel modelToJSONBlock:(TMMJSONModelKeyMapBlock)toJSON DEPRECATED_MSG_ATTRIBUTE("use initWithModelToJSONBlock:");
+ (instancetype)mapper:(TMMJSONKeyMapper *)baseKeyMapper withExceptions:(NSDictionary *)exceptions DEPRECATED_MSG_ATTRIBUTE("use baseMapper:withModelToJSONExceptions:");
...
@property (readonly, nonatomic) TMMJSONModelKeyMapBlock modelToJSONKeyBlock;
...
/**
 * Creates a TMMJSONKeyMapper instance, based on the block you provide this initializer.
 * The parameter takes in a TMMJSONModelKeyMapBlock block:
 * <pre>NSString *(^TMMJSONModelKeyMapBlock)(NSString *keyName)</pre>
 ...
 */
- (instancetype)initWithModelToJSONBlock:(TMMJSONModelKeyMapBlock)toJSON;

/**
 * Creates a TMMJSONKeyMapper instance, based on the mapping you provide.
 * Use your TMMJSONModel property names as keys, and the JSON key names as values.
 * @param toJSON map dictionary, in the format: <pre>@{@"myCamelCaseName":@"crazy_JSON_name"}</pre>
 * @return TMMJSONKeyMapper instance
 */
...
/**
 * Creates a TMMJSONKeyMapper based on a built-in TMMJSONKeyMapper, with specific exceptions.
 * Use your TMMJSONModel property names as keys, and the JSON key names as values.
 */
+ (instancetype)baseMapper:(TMMJSONKeyMapper *)baseKeyMapper withModelToJSONExceptions:(NSDictionary *)toJSON;
